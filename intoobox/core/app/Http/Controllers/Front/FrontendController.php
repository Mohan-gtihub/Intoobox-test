...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        for ($i = 1; $i <= 4; $i++) {
         ...
         foreach ($feature_category as $category) {
            // Get a random product from each featured category
            
            $randomProduct = Item::where('category_id', $category)->inRandomOrder()->first();

            if ($randomProduct) {
                $feature_category_items[] = $randomProduct;
            }
        }

        // If the total number of featured products is less than 6, you can fill the remaining slots with additional random products
        $remainingProductsCount = 6 - count($feature_category_items);

        if ($remainingProductsCount > 0) {
            $additionalRandomProducts = Item::inRandomOrder()->limit($remainingProductsCount)->get();
            $feature_category_items = array_merge($feature_category_items, $additionalRandomProducts->all());
        }
        ...
        $select_8_item_home = null;
            if ($setting->theme == 'theme2') {
                $select_8_items_home = json_decode($home_customize->home_8_popular_item, true);
                $pupular_cateogry_home4 = [];
                foreach ($select_8_items_home as $home8item) {
                    $select_8_item_home[] = Item::findOrFail($home8item);
                }
            }
        ...
        return view('front.index', [
            ...
            // Selected 8 items for home
            'select_8_item_home' => $select_8_item_home
    ...
    public function quickView($id)
    {
        ...
        $video = explode('=', $item->video);
        return view('front.catalog.quick', [
            'item'          => $item,
            'reviews'       => $item->reviews()->where('status', 1)->paginate(3),
            'galleries'     => $item->galleries,
            'video'         => $item->video ? end($video) : '',
    ...
    {
        if (Setting::first()->is_about == 0) {
            return back();
        }
        $brands=Brand::whereStatus(1)->whereIsPopular(1)->get();
        return view('front.about', compact('brands'));
    }
    ...
    {
        $catalogues=Catalogue::whereStatus(1)->get();
        return view('front.catalogue.index', compact('catalogues'));
    }


    public function coupons()
    {
        $coupons=PromoCode::whereStatus(1)->get();
        return view('front.coupons', compact('coupons'));
    }

    public function catalogueInfo($slug)
    {
        $catalogue = Catalogue::whereStatus(1)->whereSlug($slug)->firstOrFail();
        return view('front.catalogue.show', compact('catalogue'));
    }
}